...
module.exports = class Approve extends events {
  constructor(options = {}) {
    ...
    if (!options.requestMessage) options.requestMessage = '{player} te convidou para uma partida de jogo.';
    if (!options.rejectMessage) options.rejectMessage = 'O player recusou seu pedido para uma partida de jogo.';
    if (!options.reqTimeoutMessage) options.reqTimeoutMessage = 'Jogo finalizado pois o player não respondeu ao pedido.';
