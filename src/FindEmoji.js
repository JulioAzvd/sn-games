...
module.exports = class FindEmoji extends events {  
  constructor(options = {}) {
    ...
    if (!options.embed.description) options.embed.description = 'Grave bem a sequência de emojis abaixo!'
    if (!options.embed.findDescription) options.embed.findDescription = 'Encontre o emoji {emoji} antes que o tempo se esgote!';
    ...
    if (!options.winMessage) options.winMessage = 'Você conseguiu! Você selecionou corretamente o emoji. {emoji}';
    if (!options.loseMessage) options.loseMessage = 'Você não conseguiu! Você selecionou o emoji errado. {emoji}';
    if (!options.timeoutMessage) options.timeoutMessage = 'Você não conseguiu! Seu tempo se esgotou. O emoji era {emoji}';
