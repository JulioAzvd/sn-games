...
export class GuessThePokemon extends events {
  ...
  async startGame() {
    ...
    .addFields({ name: 'Tipos', value: this.pokemon.types.join(', ') ?? 'No Data', inline: true })
    .addFields({ name: 'Habilidades', value: this.pokemon.abilities.join(', ') ?? 'No Data', inline: true })
