...
export class Hangman extends events {
  ...
  async startGame() {
    ...
    .addFields({ name: `Palavra (${this.word.length} letras)`, value: this.getWordEmojis() });
  ...
  handleButtons(msg) {
    ...
    collector.on('collect', async btn => {
      ...
      .setDescription(this.getBoardContent())
      .setAuthor({ name: this.message.author.tag, iconURL: this.message.author.displayAvatarURL({ dynamic: true }) })
      .addFields({ name: 'Letras usadas', value: '`'+ this.guessed.join(', ') +'`' })
      .addFields({ name: `Palavra (${this.word.length} letras)`, value: this.getWordEmojis() });
  ...
  gameOver(msg, result) {
    ...
    if (this.guessed.length) embed.addFields({ name: 'Letras usadas', value: '`'+ this.guessed.join(', ') +'`' });
